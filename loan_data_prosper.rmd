...
monthly loan payment against the categorical variables.
...
borrowers who are employed, full-time, or self-employed, with a lot of 'not
available' values as well. Those that are part-time, retired, or not employed
largely received loans with lower monthly payments. There's also a fairly clear
negative correlation between borrower rate and monthly loan payment for those
who are employed.

Similarly, most loans are current, completed, or defaulted. A similar negative
correlation between borrower rate and monthly loan payment can be seen for
current loans. Also, interestingly, those that are past due generally have lower
monthly loan payments. This could be due to the fact that loans overall tend to
be on the lower side of monthly payments.
...
Again, there is no obvious relationship here.
...
```{r echo=FALSE}
# look at credit score and borrower rate against categorical variables
```
