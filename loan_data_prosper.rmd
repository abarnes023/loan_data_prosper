...
```{r echo=FALSE}
subset(loans, MonthlyLoanPayment > 2000)
```
...
12 months and the borrower rates are all under 0.15.
...
since it may be an entry error.
...
qplot(AmountDelinquent, data = loans,
      binwidth = 50) +
  scale_x_continuous(limits = c(0, quantile(loans$AmountDelinquent, 0.99, na.rm = T))) +
...
score
- There are some outliers in the debt to income ratio at 10.01
...
factors such as EmploymentStatus, CreditScoreRangeUpper, and StatedMonthlyIncome
...
I performed a log transformation on the StatedMonthlyIncome since the data was
skewed to the right. I wanted to see if the log transformation would normalize
the data. The transformed data did create a more normal looking distribution.
