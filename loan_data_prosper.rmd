...
any 'NA' values. Since the Prosper score contains distinct values, I want to
convert it to a factor.

The Prosper Score is also supposed to range from 1-10, so the scores
...
trend in the loans with a Prosper Score of 11 and if the data is trustworthy.
...
scores, and I made Term and Prosper score factor variables since the numerical
values fell within distinct categories. I also ordered the EmploymentStatus and
LoanStatus factors based on my perception of best to worst from the view of
the loan company.
...
ggplot(aes(x = CreditScore, y = BorrowerRate),
       data = subset(loans_subset_CreditScore, !is.na(ProsperScore))) +
  geom_jitter(aes(color = ProsperScore)) +
  scale_color_brewer(palette = 'Spectral')
```
...
The model accounts for 54.8% of the variance in the borrower rate for a loan.
...
monthly loan payment alone account for 54.8% of the variance in the borrower
rate.
...
only accounts for 54.8% of the variance in borrower rate.
