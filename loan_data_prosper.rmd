...
r <- cor(loans[, !(names(loans) %in% no_cor)], use = 'complete.obs')
round(r, 2)
```
...
pairs.panels(loans[,!(names(loans) %in% no_plot)],
             jiggle = T, factor = 1)
```
...
Let us next take a look at borrower rate vs. monthly loan payment and credit
...
the categorical variables (loan status, employment, and homeowners).
